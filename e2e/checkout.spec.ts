...
test('a customer can order for pickup and land on the tracking page', async ({ page }) => {
  await loginAs(page, 'customer')

  await page.goto(`/t/${STORE_SLUG}`)
  await expect(page.getByRole('heading', { level: 1 })).toContainText('Verde Bowl')
  ...
  await expect(page.getByRole('status')).toContainText('Nuevo')
...
test('protected checkout redirects anonymous visitors to login', async ({ page }) => {
